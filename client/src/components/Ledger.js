...
export default function Ledger({ ledger, onExport, onRefresh }) {
  const lastBal = ledger ? (ledger.entries?.[ledger.entries.length-1]?.balanceAfter ?? 0) : 0;
  return (
    <>
      <div className="flex items-center justify-between mb-2">
        <div className="text-sm text-slate-600">Running balance: <b>{ledger ? inr(lastBal) : "-"}</b></div>
          ...
          <tbody>
            {(ledger?.entries || []).map((e) => (
              <tr key={e._id} className="hover:bg-slate-50 cursor-default">
                ...
                <td className="px-3 py-2 border-t text-right">{inr(e.amount)}</td>
